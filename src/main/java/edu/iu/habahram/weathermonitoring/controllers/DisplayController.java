...
public class DisplayController {
    private final StatisticsDisplay tempDisplay;
    ...
    public ResponseEntity index() {
        ...
        html += "</li>";
        html += "<li>";
        html += String.format("<a href=/displays/%s>%s</a>", tempDisplay.id(), tempDisplay.name());
        html += "</li>";
    ...
    public ResponseEntity display(@PathVariable String id) {
        ...
        if (id.equalsIgnoreCase(tempDisplay.id())) {
            html = tempDisplay.display();
            status = HttpStatus.FOUND;
        }
        return ResponseEntity
                .status(status)
